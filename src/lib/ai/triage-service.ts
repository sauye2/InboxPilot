        ...
        input: [
          {
            role: "system",
            content:
              "You classify confidential emails for InboxPilot. Return brief, practical triage only. Do not invent facts. Choose the best category from the allowed categories based on the email thread's real intent, not only literal keywords in the newest reply. If the body contains a collapsed thread, classify using the full thread context, with the newest message used mainly to decide the next action/status. Do not demote an ongoing thread to Inbox Noise just because the newest reply is short or only asks a follow-up logistics question. Be strict about mode relevance: when an email does not belong to the selected mode, use Inbox Noise, requiresAction false, no deadline, low priority. In Recruiting mode, passive job listing alerts from LinkedIn, Jobright, Handshake, Indeed, ZipRecruiter, or similar senders belong in Recruiters, not Applications, and they should not have a deadline unless the email explicitly says apply/respond/complete by a date. Use Applications only for confirmations, status updates, or follow-ups about jobs the user applied to. In Working mode, job listings and recruiting alerts are not Manager emails and should be Inbox Noise. Only use Manager when the sender/context clearly implies a manager, lead, boss, director, supervisor, professor, or owner asking for work. Promotional emails, newsletters, sale/discount offers, upgrade prompts, feature announcements, job alerts outside Recruiting, and auth/OTP codes outside a directly relevant workflow are Inbox Noise. Purchases must be actual receipts, order confirmations, shipping, tracking, delivery, returns, subscriptions already billed, or purchased service notices, not promotional discounts. Finance must be real financial account, debt, repayment, bill, payment, statement, transaction, tax, refund, fraud, or collection messages, not upsells. Events must be real plans, invitations, reservations, tickets, appointments, RSVP requests, personal plans, or upcoming event reminders, not promotional announcements. Personal invitations like dinner, lunch, arcade, coffee, travel plans, rides, driving logistics, or 'want to go...' with a weekday/date are Events, require a reply when the latest message asks a question, and should be high priority when the event date is soon. Working mode should only contain workplace/team/client/manager/project emails; Vercel/Supabase/GitHub project/deployment/security-project emails may be Project Updates, but signup/auth/OTP/promotional emails are Inbox Noise. Deadlines, due dates, event dates, availability requests, RSVP requests, forms to submit, documents to sign, and any requested reply must raise priority only when the email is mode-relevant.",
          },
          {
            role: "user",
            content: JSON.stringify({
              ...
              email: {
                ...
                fullThreadBody: truncateForPrompt(email.body, 2400),
